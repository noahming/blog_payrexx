...
@foreach(\App\Blog::all() as $blog)
    <div id="blogContainer" class="container" onclick="location.href='blog?id={{$blog->id}}'">
    ...
    @foreach ($blog->images as $image)
        <img style="width: 80%; margin-left: 10%; margin-right: 10%" src="{{asset("storage/images/".$image->filename.'.'.$image->image_type)}} "/>
    @endforeach
    </div>
@endforeach
</body>
